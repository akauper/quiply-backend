...
from ..utils import get_framework_data_path, get_framework_path
...
class PromptManager:
    ...
    def __init__(self):
        ...
        loggers.evaluation.info(f"Loaded {len(self._flattened_prompt_dict.keys())} prompts from {len(self._loaded_files)} files.")
        if loggers.evaluation.is_debug():
            alternatives_count = 0
            for prompt in self._flattened_prompt_dict.values():
                alternatives_count += len(prompt.alternatives)
            loggers.evaluation.debug(f"And {alternatives_count} alternatives.")
        ...
        def process_dict(current_path: List[str], data: Dict):
            for k, v in data.items():
                ...
                if isinstance(v, Dict):
                    ...
                    prompt_template = EvaluablePrompt(name=name, template=v, scenario=path[0], alternatives=alternatives)
