...
from src.framework.exceptions.generation import GenerationException
from src.framework.runnables.models import RunContext
...
class TextGenerator(BaseGenerator[TextGenerationParams]):
    ...
    async def run_stream(
        ...
        loggers.framework.info(f'Called TextGeneration service {self.service_name} in {round(time.time() - start_time, 2)} seconds. Usage {response.token_usage}')
