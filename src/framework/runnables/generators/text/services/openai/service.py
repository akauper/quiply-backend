...
class OpenAiGenerationService(BaseTextGenerationService):
    ...
    def get_token_count(
        ...
        if isinstance(request, str):
            ...
            loggers.framework.warning(f"Cannot get token count for request: {request}")
            return 0
    ...
    def _get_openai_params(
        ...
        loggers.framework.dev_debug(request)
        loggers.framework.dev_debug(generation_params)
        ...
        loggers.framework.dev_debug(openai_params)
    ...
    def run(
        ...
        loggers.framework.dev_debug(chat_completion)
    ...
    async def run_stream(
        ...
        loggers.framework.dev_debug(stream)

        gen = self.converter.from_async_stream(stream)

        loggers.framework.dev_debug(gen)
