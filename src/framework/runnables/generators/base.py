...
from typing import TypeVar, Generic, Any, ClassVar, get_args, Unpack, Type, Self

from devtools import debug
from pydantic import Field, model_validator, BaseModel, ConfigDict, field_validator
...
class BaseGenerator(Runnable, ABC, Generic[TParams]):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if not hasattr(cls, 'generator_name') or cls.generator_name == "UNSET":
            raise TypeError(f"Subclasses of BaseGenerator must define a 'generator_name' class variable")
    ...
    def validate_load_settings(cls, data: Any) -> Self:
        ...
        if not cls.generator_name or cls.generator_name == "UNSET":
            raise ValueError(f"Subclasses of BaseGenerator must define a 'generator_name' class variable")

        generator_settings = framework_settings.runnables.generators.model_dump().get(cls.generator_name)
        debug(generator_settings)

        tparams_cls = cls.__pydantic_core_schema__.get('schema').get('schema').get('fields').get('generation_params').get('schema').get('cls')
