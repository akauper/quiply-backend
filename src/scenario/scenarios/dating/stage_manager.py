...
from .models import DatingStageData, DatingStage

if TYPE_CHECKING:
    from .scenario import Dating
...
class DatingStageManager(StageManager[DatingStage]):
    ...
    async def _generate_stages_async(self) -> List[DatingStage]:
        stages: List[DatingStage] = []
        for idx, agent in enumerate(self.scenario.agents):
            data = DatingStageData(
